...
;~ IniWrite ($var3, "download", "update", "http://www.vbox.me/update/")
	IniWrite($var3, "download", "key1", "https://download.virtualbox.org/virtualbox/7.2.0/VirtualBox-7.2.0-170228-Win.exe")
	IniWrite($var3, "download", "key2", "https://download.virtualbox.org/virtualbox/7.2.0/Oracle_VirtualBox_Extension_Pack-7.2.0-170228.vbox-extpack")
;~ IniWrite ($var3, "download", "update", "http://www.vbox.me/update/")
...
If (FileExists(@ScriptDir & "\app32\virtualbox.exe") Or FileExists(@ScriptDir & "\app64\virtualbox.exe")) And ($startvbox = 1 Or IniRead(@ScriptDir & "\data\settings\vboxinstall.ini", "startvbox", "key", "NotFound") = 1) Then
	...
	$sFileVer = StringLeft(FileGetVersion($arch & "\VirtualBox.exe"), 5)
	If $sFileVer >= "7.1.0" Then
		$VMTitle = "Oracle VirtualBox"
	Else
		$VMTitle = "Oracle VM VirtualBox"
	EndIf
...
;~ EndIf
      ...
      Local $USB = 0
			...
			If FileExists(@ScriptDir & "\" & $arch & "\drivers\vboxsup") And RegRead("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxSUP", "DisplayName") <> "VirtualBox Service" Then
				$SUPCmd = "cmd /c sc create VBoxSUP binpath= ""%CD%\" & $arch & "\drivers\vboxsup\VBoxSup.sys"" type= kernel start= auto error= normal displayname= PortableVBoxSUP"
        ConsoleWrite($SUPCmd & @CRLF)
        RunWait($SUPCmd, @ScriptDir, @SW_HIDE)
        ConsoleWrite("Done install VBoxSUP" & @CRLF)
				$SUP = 1
			EndIf
			...
			; Đường dẫn tool VBoxDrvInst
			Local $drvTool = @ScriptDir & "\" & $arch & "\VBoxDrvInst.exe"

      Local $ADPVER = ""

			If IniRead($var1, "net", "key", "NotFound") = 1 Then
				If FileExists(@ScriptDir & "\" & $arch & "\drivers\network\netadp6") Then
					$ADPVER = 6
				EndIf
...
;~ EndIf
				...
				If RegRead("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxNetAdp", "DisplayName") <> "VirtualBox Host-Only Network Adapter" Then
					RunWait('"' & $drvTool & '" install --inf-file "' & @ScriptDir & '\' & $arch & '\drivers\network\netadp6\VBoxNetAdp6.inf"', @ScriptDir, @SW_HIDE)
					Local $ADP = 1
				Else
					Local $ADP = 0
				EndIf
			...
			; Host-Only Adapter
;~ RunWait('"' & $drvTool & '" install --inf-file "' & @ScriptDir & '\' & $arch & '\drivers\network\netadp6\VBoxNetAdp6.inf"', @ScriptDir, @SW_HIDE)
;~ RunWait('"' & $drvTool & '" install --inf-file "' & @ScriptDir & '\' & $arch & '\drivers\network\netflt\VBoxNetFlt.inf"', @ScriptDir, @SW_HIDE)
;~ RunWait('"' & $drvTool & '" install --inf-file "' & @ScriptDir & '\' & $arch & '\drivers\network\netlwf\VBoxNetLwf.inf"', @ScriptDir, @SW_HIDE)

      Local $NET = 0
			If IniRead($var1, "net", "key", "NotFound") = 1 Then
				If RegRead("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxNetLwf", "DisplayName") <> "VBoxNetLwf Service" Then
;~ If @OSArch = "x86" Then
...
;~ RunWait('"' & $drvTool & '" install --inf-file "' & @ScriptDir & '\' & $arch & '\drivers\network\netlwf\VBoxNetLwf.inf"', @ScriptDir, @SW_HIDE)
					...
					;~ $colItems = $oWMI.ExecQuery( _
					;~ 		"SELECT Name, PNPDeviceID, PhysicalAdapter FROM Win32_NetworkAdapter WHERE PhysicalAdapter = TRUE" _
					;~ 		)

					;~ For $oItem In $colItems
					;~ 	If StringLeft($oItem.PNPDeviceID, 4) = "PCI\" Then
					;~ 		ConsoleWrite("Name: " & $oItem.Name & @CRLF)
					;~ 		ConsoleWrite("PNPDeviceID: " & $oItem.PNPDeviceID & @CRLF)
					;~ 		ConsoleWrite("---------------------------" & @CRLF)
...
;~ EndIf
		...
		If FileExists($SourceFile) Then
			...
			Opt("WinTitleMatchMode", 2)
			ConsoleWrite("WinWait@CRLF")
			WinWait("VirtualBox Installer", "")
			ControlClick("VirtualBox Installer", "OK", "Button")
			WinClose("VirtualBox Installer", "")
		EndIf
	EndIf
...
;~   RunWait ('"'&@ScriptDir&'\data\tools\7za.exe"'&" x -o"&'"'&@ScriptDir&'\temp\ExtensionPacks\Oracle_VM_VirtualBox_Extension_Pack\"'&" "&'"'&@ScriptDir&'\temp\Extension~"', @ScriptDir, @SW_HIDE)
;~ EndIf
	; Lấy thư mục chứa file nguồn
	$PatchFolder = StringRegExpReplace($SourceFile, "\\[^\\]+$", "")

	; Trường hợp 1: Extension nằm cùng thư mục với SourceFile
	If FileExists($PatchFolder & "\Extension") Then
		NestedExtract($PatchFolder & "\Extension")
	EndIf

	; Trường hợp 2: Extension nằm ngay thư mục script
	If FileExists(@ScriptDir & "\Extension") Then
		NestedExtract(@ScriptDir & "\Extension")
	EndIf
	...
	If GUICtrlRead($Checkbox100) = $GUI_CHECKED And FileExists(@ScriptDir & "\temp") Then
		...
		DirCopy(@ScriptDir & "\temp\ExtensionPacks\Oracle_VM_VirtualBox_Extension_Pack", @ScriptDir & "\app32\ExtensionPacks\Oracle_VirtualBox_Extension_Pack", 1)
		FileCopy(@ScriptDir & "\temp\x86\PFiles\Oracle\VirtualBox\*", @ScriptDir & "\app32", 9)
		DirRemove(@ScriptDir & "\app32\ExtensionPacks\Oracle_VirtualBox_Extension_Pack\darwin.amd64", 1)
		DirRemove(@ScriptDir & "\app32\ExtensionPacks\Oracle_VirtualBox_Extension_Pack\darwin.arm64", 1)
		DirRemove(@ScriptDir & "\app32\ExtensionPacks\Oracle_VirtualBox_Extension_Pack\linux.amd64", 1)
		DirRemove(@ScriptDir & "\app32\ExtensionPacks\Oracle_VirtualBox_Extension_Pack\solaris.amd64", 1)
	...
	If GUICtrlRead($Checkbox110) = $GUI_CHECKED And FileExists(@ScriptDir & "\temp") Then
		...
		DirCopy(@ScriptDir & "\temp\ExtensionPacks\Oracle_VM_VirtualBox_Extension_Pack", @ScriptDir & "\app64\ExtensionPacks\Oracle_VirtualBox_Extension_Pack", 1)
		FileCopy(@ScriptDir & "\temp\x64\PFiles\Oracle\VirtualBox\*", @ScriptDir & "\app64", 9)
		DirRemove(@ScriptDir & "\app64\ExtensionPacks\Oracle_VirtualBox_Extension_Pack\darwin.amd64", 1)
		DirRemove(@ScriptDir & "\app64\ExtensionPacks\Oracle_VirtualBox_Extension_Pack\darwin.arm64", 1)
		DirRemove(@ScriptDir & "\app64\ExtensionPacks\Oracle_VirtualBox_Extension_Pack\linux.amd64", 1)
		DirRemove(@ScriptDir & "\app64\ExtensionPacks\Oracle_VirtualBox_Extension_Pack\solaris.amd64", 1)
...
; Hàm giải nén lồng nhau
Func NestedExtract($ExtensionPath)
	Local $TempFolder = @ScriptDir & "\temp\"
	Local $InnerFile = $TempFolder & "Extension~"
	Local $DestFolder = $TempFolder & "ExtensionPacks\Oracle_VM_VirtualBox_Extension_Pack\"

	; Bước 1: Giải nén lớp ngoài
	If FileExists($ExtensionPath) Then
		RunWait('"' & @ScriptDir & '\data\tools\7za.exe" x -y -o"' & $TempFolder & '" "' & $ExtensionPath & '"', @ScriptDir, @SW_HIDE)
	Else
		ConsoleWrite("[ERROR] Extension path not found: " & $ExtensionPath & @CRLF)
		Return
	EndIf

	; Kiểm tra xem file Extension~ có được tạo ra không
	If FileExists($InnerFile) Then
		; Bước 2: Giải nén lớp bên trong
		RunWait('"' & @ScriptDir & '\data\tools\7za.exe" x -y -o"' & $DestFolder & '" "' & $InnerFile & '"', @ScriptDir, @SW_HIDE)
	Else
